...
export type ZoomableMediaOnTransform = NonNullable<ReactZoomPanPinchProps["onTransform"]>;

export function useZoomableMedia() {
  const ref = useRef<ReactZoomPanPinchContentRef>(null);
  const [scale, setScale] = useState(1);

  const onTransform = useCallback<ZoomableMediaOnTransform>((_instance, state) => {
