...
#[tauri::command] pub async fn list_tags(state: State<'_, AppState>) -> Result<Vec<TagDto>> {
    let _correlation_id = begin_command("list_tags");
    state
        .with_active(|ws| async move {
            let rows = TagRepo::new(ws.catalog.pool().clone()).list_tag_rows().await?;
            Ok(rows
                .into_iter()
                .map(|row| TagDto {
                    id: row.id,
...
#[tauri::command] pub async fn create_tag(
    ...
    let _correlation_id = begin_command("create_tag");
    state
        .with_active(|ws| async move {
...
#[tauri::command] pub async fn update_tag(
    id: i64,
    name: String,
    color: Option<String>,
    state: State<'_, AppState>,
) -> Result<TagDto> {
    let _correlation_id = begin_command("update_tag");
    state
        .with_active(|ws| async move {
...
#[tauri::command] pub async fn delete_tag(id: i64, state: State<'_, AppState>) -> Result<()> {
    let _correlation_id = begin_command("delete_tag");
    state
        .with_active(|ws| async move {
            let pool = ws.catalog.pool().clone();
            let repo = TagRepo::new(pool.clone());
...
#[tauri::command] pub async fn batch_append_tags<R: Runtime>(
    asset_ids: Vec<i64>,
    tag_id: i64,
    app: AppHandle<R>,
    state: State<'_, AppState>,
) -> Result<u64> {
    let correlation_id = begin_command("batch_append_tags");
    ...
    let count = state
        .with_active(|ws| async move {
            let count = ws
                ...
                .await?;
            if count > 0 { record_tags_added(&ws.activity, Some(&correlation_for_ws), &asset_ids, tag_id).await?; }
            Ok(count)
        })
        .await?;
...
#[tauri::command] pub async fn batch_remove_tags<R: Runtime>(
    asset_ids: Vec<i64>,
    tag_id: i64,
    app: AppHandle<R>,
    state: State<'_, AppState>,
) -> Result<u64> {
    let correlation_id = begin_command("batch_remove_tags");
    let correlation_for_ws = correlation_id.clone();
    let count = state
        .with_active(|ws| async move {
            let count = ws
                ...
                .await?;
            if count > 0 { record_tags_removed(&ws.activity, Some(&correlation_for_ws), &asset_ids, tag_id).await?; }
            Ok(count)
        })
        .await?;
